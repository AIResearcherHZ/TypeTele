...
def create_leap_node(cfg):
    if cfg.get("sim", False):
        from leap_hand_utils.leap_node_sim import LeapNodeSim

        return LeapNodeSim(cfg)
    return LeapNode(cfg)
...
class LeapNode:
    def __init__(self, cfg):
        ...
        if self.init_pos is not None:
            ...
            self.prdev_pos = self.pos = self.curr_pos = lhu.allegro_to_LEAPhand(
                np.zeros(16)
            )
